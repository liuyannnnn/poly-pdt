...
- 调 GGS HTTP 接口采集或查询近期足球比赛，用于外部数据源匹配。
- 以 PM 为基准完成 PM/GS/GGS 匹配。
- 生成全局唯一 `guid`。
...
PM 数据、GS 数据和 GGS 数据必须分开：

- PM 当前态写 `pm:match:{guid}`。
- GS 当前态写 `gs:match:{guid}`。
- GGS 当前态写 `ggs:match:{guid}`。
...
- 同一交易员同一比赛的持仓 tick 检查只允许一个后台处理任务运行；当前处理中的 tick 保留，后续等待 tick 只保留最新一条，用于避免 task 堆积。
- 比赛结束后或结束 15 分钟后，行情重采样器应清理该比赛的最新 tick / md5 内存态；Redis 历史仍按 TTL 保留。
...
- GS 或 GGS 等外部数据源比分变化早于 PM base 比分变化时触发。
