...
- 同一交易员同一比赛的 tick 处理只保留一个运行任务；正在处理的 tick 不覆盖，等待中的 tick 只保留最新一条。
- 账号事件按 `provider + account_alias` 路由，不按 `guid` 参与行情或比赛信号队列。
- 所有 Redis 历史时序和日志继续追加写入，不重写旧数据。
- 追加 list/stream 时发现旧 key 类型不符，报错并写系统日志，不自动删除或迁移历史 key。
- 重采样器、交易员后台循环等异常必须写 `stream:system_logs`，不得静默吞掉。
- 比赛结束后清理该比赛的重采样内存态；Redis 历史依然按 TTL 保留。
- 交易员启动只恢复账户、持仓和配置；历史交易记录/日志按 API 分页从 Redis stream 读取，不全量常驻内存。
